...
#define nAPPID          @"5cd069b07b64cf57e927cbabf5a26b7a"
...
/**********************************************************************************************/
#pragma mark - json custom methods
/**********************************************************************************************/
...
+ (NSDictionary*) sendPost:(NSString*)postUrl forData:(NSString *)data andMode:(BOOL)mode {
    @try {
        ...
        print(NSLog(@"post parameters: %@",post)) // Debug message
        post                    = [post stringByReplacingOccurrencesOfString:@"+" withString:@"%2B"]; // Replace '+' ocurrences for white spaces.
        NSURL *url              = [NSURL URLWithString:postUrl]; // URL object init.
        print(NSLog(@"URL post  = %@", url)) // Debug message
        NSData *postData        = [post dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES]; // Enable encoding UTF8
        NSString *postLength    = [NSString stringWithFormat:@"%lu", (unsigned long)[postData length]];
        
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init]; // Do the request to the URL
        [request setURL:url];
        ...
        [request setValue:@"UAG_1.0" forHTTPHeaderField:@"User-Agent"]; // Only matters when backend is validating the user agent.
        ...
        else { if (error) {print(NSLog(@"Error response")) return nil; } else {print(NSLog(@"Connect Fail"))return nil;}
            return nil;
        }
    }
